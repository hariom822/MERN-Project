  ...
  const formatted = res.data
    ...
    .flatMap(t =>
      t.schedule.map(s => {
        ...
        return {
          id: t._id,
          title: `${t.course?.coursename} - ${s.day} (${s.time})`,
          start: start.toDate(),
...
const handleDelete = async () => {
  ...
  try {
    await axios.delete(
      `http://localhost:8000/timetable/delete/${selectedEvent.id}`
    );
      ...
      <div className="bg-white p-6 rounded-2xl shadow-md mb-6 flex justify-between items-center">
        ...
        <div className="flex gap-4">
          ...
          {selectedCourse && (
            ...
            >
              <option value="">Select Batch</option>
             {batches.filter(b => String(b.course?._id || b.course) === String(selectedCourse))
               .map(b =>  (
                <option key={b._id} value={b._id}>
                  {b.batchName}
                </option>
  ...
  <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center z-50">
    ...
    <div className="bg-white w-full max-w-md rounded-2xl p-6 shadow-2xl">
      ...
      <div className="space-y-2 text-gray-700">
        ...
        <p><strong>Course:</strong> {selectedEvent.fullData.course?.coursename}</p>

        <p><strong>Batch:</strong> {selectedEvent.fullData.batch?.batchName}</p>

        <p><strong>Day:</strong> {selectedEvent.day}</p>

        <p><strong>Time:</strong> {selectedEvent.time}</p>

        <p><strong>Tutor:</strong> {selectedEvent.fullData.tutor?.name}</p>
